...
export const calculateAGPMetrics = (data: CGMData[], analysisPeriod: AnalysisPeriod, breakpoints: Breakpoints): AGPMetrics => {
...
export interface AnalysisPeriod {
   start: string; end: string, trailingDays?: number,
}
...
export const generateAGPReportBundle = (
  { bundle, analysisPeriod = {trailingDays: 120}, includeSourceData =  true, targetUnit = "mg/dL", breakpoints = undefined}: GenerateAGPReportArgs
): FHIRBundle => {
  ...
  if (analysisPeriod.start && analysisPeriod.end) {
    startDate = moment(analysisPeriod.start);
    endDate = moment(analysisPeriod.end);
  } else if (analysisPeriod.trailingDays) {
    endDate = moment();
    startDate = moment().subtract(analysisPeriod.trailingDays, "days");
  } else {
  ...
  const filteredObservations: { fullUrl: string; resource: FHIRObservation }[] = bundle.entry
    .filter((entry) => entry.resource.resourceType === "Observation")
    .filter(({ resource: observation }) => {
      const obs = observation as FHIRObservation;
      return obs.code.coding.some(c => c.code === "99504-3") &&
        obs.valueQuantity &&
  ...
  const glucoseObservations: CGMData[] = filteredObservations
    ...
    const agpMetrics = calculateAGPMetrics(glucoseObservations, {start: startDate.toISOString(), end: endDate.toISOString()}, breakpoints || makeBreakpoints(targetUnit));
  ...
  const agpObservation: FHIRObservation = {
    ...
    component: [
      ...
      createComponent("https://tx.argo.run", "time-in-low", "Time in Low Range", agpMetrics.timeInRanges.low, "%", "%"),
      createComponent(
        "https://tx.argo.run",
        "time-in-target",
      ...
      createComponent("https://tx.argo.run", "time-in-high", "Time in High Range", agpMetrics.timeInRanges.high, "%", "%"),
      createComponent(
        "https://tx.argo.run",
        "time-in-very-high",
      ...
      createComponent( "https://tx.argo.run",
        "cv",
